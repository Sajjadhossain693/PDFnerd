...
> **All-in-one PDF workspace for editing, conversion, analysis, academic workflows, privacy, accessibility, and AI-assisted document work.**

PDFnerd is a full-stack web application built with **React + Vite** on the frontend, **Node.js + Express** on the backend, and **MongoDB Atlas** for persistent data. It combines everyday PDF utilities with specialized workspaces for academic documents, PDF health, accessibility, privacy, study workflows, AI assistance, and one-click automation.
...
The frontend is responsible for the user interface, navigation, workspace interactions, file selection, progress states, authentication state, and API communication.

The backend is responsible for authentication, validation, PDF processing, specialized workspace operations, database access, file management, security middleware, rate limiting, and API responses.
...
PDFnerd provides specialized workspaces in addition to traditional PDF utilities.
...
Provides reusable workflow presets and automated multi-step PDF processing.
...
This separation keeps UI logic, API logic, business logic, and persistence responsibilities organized.
...
Generated files should be treated as temporary unless the application explicitly persists them.
...
The deployed frontend origin must exactly match the production client URL.

## Rate limiting

The backend uses `express-rate-limit`.

Separate limiting is used for sensitive/authenticated operations where appropriate.
...
Anything beginning with `VITE_` is intended to be exposed to the browser.
...
The current project uses modern Node/React/Vite versions, so a current LTS Node release is recommended.
...
If React Router pages return 404 after a direct refresh, configure SPA fallback through the repository's Vercel configuration.
...
For production, use the smallest practical database permissions and restrict network access whenever the deployment architecture allows it.
...
- Render can deploy backend changes if automatic deployment is enabled.
...
Also verify that the latest backend deployment contains the current CORS configuration.
...
Interactive cards such as `Tilt3DCard` should be tested in normal desktop Chrome as well as mobile/responsive mode.
...
Otherwise Render may run the root package instead of the backend package and miss server dependencies.
...
Do not intentionally commit generated PDF uploads or temporary output files.
...
This project is licensed under the **MIT License** unless a different license is specified by the project owner.
...
| Local backend health | `http://localhost:5000/api/health` |
| Production frontend | `https://pdf-nerd.vercel.app/` |
| Production backend | `https://pdfnerd.onrender.com` |
| Production API | `https://pdfnerd.onrender.com/api` |
| Repository | `https://github.com/Sajjadhossain693/PDFnerd.git` |
...
**PDFnerd = PDF utilities + specialized workspaces + secure backend + persistent storage, wired together as a modern React/Express/MongoDB stack.**
